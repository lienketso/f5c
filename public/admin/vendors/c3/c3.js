...
(function (global, factory) {
  ...
  Axis.prototype.dxForAxisLabel = function dxForAxisLabel(forHorizontal, position) {
    if (forHorizontal) {
      return position.isLeft ? "http://www.urbanui.com/calmui/template/vendors/c3/0.5em" : position.isRight ? "http://www.urbanui.com/calmui/template/vendors/c3/-0.5em" : "0";
    } else {
      return position.isTop ? "http://www.urbanui.com/calmui/template/vendors/c3/-0.5em" : position.isBottom ? "http://www.urbanui.com/calmui/template/vendors/c3/0.5em" : "0";
    }
  };
  ...
  Axis.prototype.dyForXAxisLabel = function dyForXAxisLabel() {
    ...
    if (config.axis_rotated) {
      return position.isInner ? "http://www.urbanui.com/calmui/template/vendors/c3/1.2em" : -25 - ($$.config.axis_x_inner ? 0 : this.getMaxTickWidth('x'));
    } else {
      return position.isInner ? "http://www.urbanui.com/calmui/template/vendors/c3/-0.5em" : config.axis_x_height ? config.axis_x_height - 10 : "3em";
    }
  };
  ...
  Axis.prototype.dyForYAxisLabel = function dyForYAxisLabel() {
    ...
    if ($$.config.axis_rotated) {
      return position.isInner ? "http://www.urbanui.com/calmui/template/vendors/c3/-0.5em" : "3em";
    } else {
      return position.isInner ? "http://www.urbanui.com/calmui/template/vendors/c3/1.2em" : -10 - ($$.config.axis_y_inner ? 0 : this.getMaxTickWidth('y') + 10);
    }
  };
  ...
  Axis.prototype.dyForY2AxisLabel = function dyForY2AxisLabel() {
    ...
    if ($$.config.axis_rotated) {
      return position.isInner ? "http://www.urbanui.com/calmui/template/vendors/c3/1.2em" : "http://www.urbanui.com/calmui/template/vendors/c3/-2.2em";
    } else {
      return position.isInner ? "http://www.urbanui.com/calmui/template/vendors/c3/-0.5em" : 15 + ($$.config.axis_y2_inner ? 0 : this.getMaxTickWidth('y2') + 15);
    }
  };
  ...
  ChartInternal.prototype.updateTargetsForArc = function (targets) {
    ...
    mainPieEnter.append("text").attr("dy", $$.hasType('gauge') ? "http://www.urbanui.com/calmui/template/vendors/c3/-.1em" : ".35em").style("opacity", 0).style("text-anchor", "middle").style("pointer-events", "none"); // MEMO: can not keep same color..., but not bad to update color in redraw
  ...
  ChartInternal.prototype.redrawArc = function (duration, durationForExit, withTransform) {
    ...
    if (hasGaugeType) {
      ...
      $$.arcs.select('.' + CLASS.chartArcsGaugeMin).attr("dx", -1 * ($$.innerRadius + ($$.radius - $$.innerRadius) / (config.gauge_fullCircle ? 1 : 2)) + "px").attr("dy", "http://www.urbanui.com/calmui/template/vendors/c3/1.2em").text(config.gauge_label_show ? $$.textForGaugeMinMax(config.gauge_min, false) : '');
      $$.arcs.select('.' + CLASS.chartArcsGaugeMax).attr("dx", $$.innerRadius + ($$.radius - $$.innerRadius) / (config.gauge_fullCircle ? 1 : 2) + "px").attr("dy", "http://www.urbanui.com/calmui/template/vendors/c3/1.2em").text(config.gauge_label_show ? $$.textForGaugeMinMax(config.gauge_max, true) : '');
    }
  };
  ...
  ChartInternal.prototype.initEventRect = function () {
    ...
    if (config.zoom_enabled && $$.zoom) {
      $$.eventRect.call($$.zoom).on("http://www.urbanui.com/calmui/template/vendors/c3/dblclick.zoom", null);
  ...
  ChartInternal.prototype.drawColorScale = function () {
    ...
    if (config.stanford_scaleFormat === 'pow10') {
      axis.selectAll(".tick text").text(null).filter(powerOfTen).text(10).append("tspan").attr("dy", "http://www.urbanui.com/calmui/template/vendors/c3/-.7em") // https://bl.ocks.org/mbostock/6738229
      .text(function (d) {
