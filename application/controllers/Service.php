...
Class Service extends MY_Controller{
	...
	function index(){
		$input = array();
		//tour nổi bật
		$input['where'] = ['status'=>1,'lang_code'=>$this->language,'type'=>'service','display'=>1];
		$input['limit'] = [4,0];
		$listHot = $this->news_model->get_list($input);
		$this->data['listHot'] = $listHot;

		$cat['where'] = ['status'=>1,'display'=>1,'cat_type'=>'service','lang_code'=>$this->language];
		$cat['order'] = ['is_order','asc'];
		$cat['limit'] = [1,0];
		$catdanhmuc = $this->catnews_model->get_list($cat);
		$this->data['catdanhmuc'] = $catdanhmuc;

		$cat2['where'] = ['status'=>1,'display'=>2,'cat_type'=>'service','lang_code'=>$this->language];
		$cat2['order'] = ['is_order','asc'];
		$cat2['limit'] = [1,0];
		$catsanpham = $this->catnews_model->get_list($cat2);
		$this->data['catsanpham'] = $catsanpham;


        //input to header
        $this->data['meta_title'] = 'Sản xuất';
        $this->data['meta_keyword'] = 'Dịch vụ sản xuất';
        ...
        $this->data['canonical'] = '';
        //pre($catdanhmuc);die;
		$this->data['temp'] = 'site/templates/service/index';
