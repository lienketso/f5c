<?php 
Class Real extends MY_Controller{
	...
	function index(){
		$input = array();
		//tour nổi bật
		$input['where'] = ['status'=>1,'lang_code'=>$this->language,'type'=>'real','display'=>1];
		$input['limit'] = [4,0];
		$listHot = $this->news_model->get_list($input);
		$this->data['listHot'] = $listHot;

		$cat['where'] = ['status'=>1,'cat_type'=>'real','lang_code'=>$this->language];
		...
		$this->data['temp'] = 'site/templates/real/index';
	...
	function dangky(){
		...
		$data = array(
			...
			'user_phone'=>$phone,
			'user_email'=>$email,
			'title' =>'Đăng ký nhận tin bất động sản',
			'created'=>$created
		);
		try{
