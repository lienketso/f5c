...
<input type="text" name="url" value="<?= set_value('url'); ?>" class="form-control" placeholder="Nhập link liên kết">
<span id="" class="error mt-2 text-danger" for=""><?php echo form_error('url'); ?></span>
</div>
<div class="form-group">
<label for="">Thứ tự</label>
<input type="text" name="sort_order" value="<?= set_value('sort_order'); ?>" class="form-control" placeholder="Thứ tự hiển thị">
...
<select name="ads_location_id" disabled="disabled" class="form-control form-control-sm" id="">
	<option value="0">---Chọn vị trí---</option>
	<?php foreach($listLocation as $row): ?>
	<option value="<?= $row->id; ?>" <?= ($row->id==$gid) ? 'selected' : ''; ?> ><?= $row->name; ?></option>
<?php endforeach; ?>
</select>
</div>
<div class="form-group">
