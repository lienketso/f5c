...
<div class="card">
<div class="card-body">
<h4 class="card-title">Nhập thông tin để sửa nhân viên hỗ trợ</h4>
...
<input type="text" name="phone" value="<?= $info->phone; ?>" class="form-control" placeholder="Số di động">
<span id="" class="error mt-2 text-danger" for=""><?php echo form_error('phone'); ?></span>
</div>
<div class="form-group">
<label for="">Điện thoại</label>
<input type="text" name="mobile" value="<?= $info->mobile; ?>" class="form-control" placeholder="Số máy khác">
<span id="" class="error mt-2 text-danger" for=""><?php echo form_error('mobile'); ?></span>
</div>
<div class="form-group">
<label for="">Email</label>
<input type="text" name="email" value="<?= $info->gmail; ?>" class="form-control" placeholder="Địac chỉ email">
...
<select name="group_id" class="form-control form-control-sm" id="">
	...
	<option value="<?= $row->id ?>" <?= ($row->id==$info->group_id) ? 'selected' : '' ?> ><?= $row->name; ?></option>
	<?php endforeach; ?>
