...
<li class="breadcrumb-item active" aria-current="page">Danh sách vận tải</li>
</ol>
</nav>
...
<a href="<?= admin_url('news?type=car'); ?>" class="btn btn-secondary">Làm lại</a>
</div>
</div>
</form>
</div>
...
<a href="<?php echo admin_url('forum/add?type='.$type); ?>" class="btn btn-small btn-success"><i class="ti-write"></i> Thêm mới</a>
...
<?php foreach($list as $row) : ?>
	<?php 
	$user = $this->user_model->get_info($row->member_id);
	if(!empty($user)){
		$member = $user->email;
	}else{
		$member = '';
...
<td><span style="font-weight: bold;"><?= $row->view; ?></span></td>
<td class="center"><?php echo format_date($row->created_at); ?></td>
<td class="center">
<a class="btn btn-sm btn-info" href="<?php echo admin_url('forum/edit/'.$row->id.'?type='.$type); ?>">
<i class="ti-pencil-alt"></i>  
</a>
<a class="btn btn-sm btn-danger" href="<?php echo admin_url('forum/del/'.$row->id.'?type='.$type); ?>" onclick="return check_del();">
<i class="ti-trash"></i>  
</a>
</td>
</tr>
